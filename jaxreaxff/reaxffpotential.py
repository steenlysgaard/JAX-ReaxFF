...
def calculate_vdw_pot(atom_types,atom_mask,distance_matrices,tapering_matrices, p1co, p2co, p3co, vop, vdw_shiedling):
    ...
    self_multip = np.asarray(self_multip).at[jax.numpy.index_exp[di]].set(0.5)
...
def calculate_eem_charges(atom_types,atom_mask,total_charge,hulp1_mat,tapering_matrices, gamma, idempotential, electronegativity):
    ...
    A = np.asarray(A).at[jax.numpy.index_exp[num_atoms, :num_atoms]].set(atom_mask)
    A = np.asarray(A).at[jax.numpy.index_exp[:num_atoms, num_atoms]].set(atom_mask)
    ...
    di = np.diag_indices(num_atoms)
    A_sub_mat = np.asarray(A_sub_mat).at[jax.numpy.index_exp[di]].set(2.0 * idempotential[atom_types] + A_sub_mat[di])

    A = np.asarray(A).at[jax.numpy.index_exp[:num_atoms,:num_atoms]].set(A_sub_mat)

    A = np.asarray(A).at[jax.numpy.index_exp[num_atoms, num_atoms]].set(0.0)
    #A[num_atoms][num_atoms] = 0.0
    # create b
    b = np.zeros(shape=(num_atoms+1), dtype=TYPE)
    #b[-1] = 0 # total charge is 0
    b = np.asarray(b).at[jax.numpy.index_exp[:num_atoms]].set(-1 * electronegativity[atom_types])
    # total charge
    b = np.asarray(b).at[jax.numpy.index_exp[num_atoms]].set(total_charge)
    ...
    atom_charges = np.asarray(atom_charges).at[jax.numpy.index_exp[:-1]].set(atom_charges[:-1] * atom_mask)
    return atom_charges
